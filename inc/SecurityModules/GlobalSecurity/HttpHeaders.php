...
class HttpHeaders {
	...
	private function __construct() {
		...
		add_action( 'wp_headers', array( self::class, 'remove_wp_headers' ), 10, 1 );

		add_action( 'template_redirect', array( self::class, 'send_headers_early' ), 1 );

		add_action( 'admin_init', array( self::class, 'send_headers_early' ), 1 );

		add_action( 'login_init', array( self::class, 'send_headers_early' ), 1 );

		add_action( 'do_feed', array( self::class, 'send_headers_early' ), 1 );
		add_action( 'do_feed_rss', array( self::class, 'send_headers_early' ), 1 );
		add_action( 'do_feed_rss2', array( self::class, 'send_headers_early' ), 1 );
		add_action( 'do_feed_atom', array( self::class, 'send_headers_early' ), 1 );

		add_action( 'wp', array( self::class, 'send_headers_on_404' ), 1 );
	}
	...
	public static function get_all_headers_options(): array {
		return array(
			'secure_headers'      => self::get_secure_headers_options(),
			'caching_headers'     => self::get_caching_headers_options(),
