...
class RateLimiter {
    ...
    public static function inspect($origin = 'public_rate_limit') {
        ...
        $rate_data = self::increment_request_count(
            $client_ip,
            $time_window
        );
        
        $count = $rate_data['count'];
        $retry_after = $rate_data['retry_after'];
        ...
        return new WP_Error(
            ...
            array( 'status' => 429, 'headers' => array( 'Retry-After' => (string) $retry_after ) )
        );
    }
    ...
    private static function increment_request_count(
        ...
        $retry_after = max( 1, $window - ( $now - $data['window_start'] ) );
