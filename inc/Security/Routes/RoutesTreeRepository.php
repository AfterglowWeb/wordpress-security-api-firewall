...
class RoutesTreeRepository {
    ...
    private static function merge_node_with_saved(array $node, ?array $saved_node): array {
        ...
        foreach ($saved_node as $key => $value) {
            if (!in_array($key, ['settings', 'children', 'permission'], true)) {
                $merged[$key] = $value;
            }
        }
        
        foreach (['settings', 'permission'] as $key) {
            if (isset($saved_node[$key])) {
                $merged[$key] = self::merge_recursive($merged[$key] ?? [], $saved_node[$key]);
            }
        }
    ...
    public static function sanitize_routes_policy_tree($tree): array {
        ...
        return array_values(array_filter(array_map([self::class, 'sanitize_node'], $tree)));
    }
    ...
    private static function sanitize_node(array $node): array {
        ...
        foreach ($node as $key => $value) {
            switch ($key) {
                ...
                case 'params':
                    $sanitized[$key] = is_array($value) ? self::sanitize_params($value) : [];
                    break;
                    
                case 'settings':
                    $sanitized[$key] = is_array($value) ? self::sanitize_settings($value) : [];
                    break;
                ...
                case 'children':
                    $sanitized[$key] = is_array($value) ? self::sanitize_routes_policy_tree($value) : [];
                    break;
    ...
    private static function sanitize_settings(array $settings): array {
        ...
        foreach ($settings as $key => $value) {
            if (in_array($key, ['disabled', 'protect'], true) && is_array($value)) {
                $sanitized[$key] = [
    ...
    private static function get_saved_rest_routes(): array {
        ...
        return is_array($saved) ? self::sanitize_routes_policy_tree($saved) : [];
    }
}